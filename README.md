...
- Support for Terraform >= 0.12 is currently provided as a beta feature. You will need to select the config setting for it. Note that only `validate` linting functionality changes in >= 0.12. Note also that config settings related to variables are ignored in Terraform >= 0.12 `validate`.
