    ...
    return {
      ...
      lint: (activeEditor) => {
        ...
        if (atom.config.get('linter-terraform-syntax.useTerraformFormat')) {
          ...
          const fmt_target = atom.config.get('linter-terraform-syntax.formatCurrentFile') ? file : ''
          // auto-format the target
          helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), ['fmt', '-list=false', fmt_target], { cwd: dir })
        }
