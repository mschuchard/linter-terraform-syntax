      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), args, {cwd: dir, stream: 'stderr', allowEmptyStderr: true}).then(output => {
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            if (matches_alt_syntax != null && matches_file != null) {
              toReturn.push({
                severity: 'error',
                excerpt: matches_alt_syntax[1] + matches_alt_syntax[4],
                location: {
                  file: file,
                  position: [[Number.parseInt(matches_alt_syntax[2]) - 1, Number.parseInt(matches_alt_syntax[3]) - 1], [Number.parseInt(matches_alt_syntax[2]) - 1, Number.parseInt(matches_alt_syntax[3])]],
                },
              });
            }
