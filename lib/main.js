...
export default {
  config: {
    ...
    globalVarFiles: {
      ...
      type: 'array',
      description: 'Var files specified by absolute paths that should be applied to all projects. (plan only)',
      default: [''],
      items: {
        type: 'string'
      }
    ...
    localVarFiles: {
      ...
      type: 'array',
      description: 'Var files specified by relative paths to each project that should be applied. If these files are not in the same relative path within each project this will fail. (plan only)',
      default: [''],
      items: {
        type: 'string'
      }
    ...
    checkRequiredVar: {
      ...
      type: 'boolean',
      description: 'Check whether all required variables have been specified (unchecking is useful if primarily developing/declaring modules; only works with validate and < 0.12).',
      default: true,
    },
    timeout: {
  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        var options = { cwd: dir, stream: 'stderr', allowEmptyStderr: true, timeout: atom.config.get('linter-terraform-syntax.timeout') * 1000 }

        // json output for validate if >= 0.12
        if (!(atom.config.get('linter-terraform-syntax.useTerraformPlan'))) {
          args.push('-json')
          // stdout for json output so also have to ignore exit code
          // some js bug prevents object key manipulation here so reassign var
          options = { cwd: dir, ignoreExitCode: true, timeout: atom.config.get('linter-terraform-syntax.timeout') * 1000 }
        }
        ...
        if (atom.config.get('linter-terraform-syntax.useTerraformFormat')) {
          ...
          const fmt_target = atom.config.get('linter-terraform-syntax.formatCurrentFile') ? file : dir
          // auto-format the target
          helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), ['fmt', '-list=false', fmt_target])
        }
