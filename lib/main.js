...
export default {
    ...
    format: {
      ...
      properties: {
        ...
        currentFile: {
          ...
          description: 'Only format the currently opened file instead of all files in the directory. Mutually exclusive with "recursive".',
          type: 'boolean',
          default: false,
        },
        recursive: {
          title: 'Recursive Format',
          description: 'Recursively format all Terraform files from the directory of the current file. Mutually exclusive with "current file".',
          type: 'boolean',
          default: false,
        },
      },
  ...
  activate() {
    ...
    this.helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), ['validate', '--help']).then(output => {
      if (!(/-json/.exec(output))) {
  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        // auto-formatting
        if (atom.config.get('linter-terraform-syntax.format.enabled')) {
          const fmtArgs = ['fmt', '-list=false', '-no-color'];

          // recursive format if selected...
          if (atom.config.get('linter-terraform-syntax.format.recursive'))
            fmtArgs.push('-recursive');
          // ... or the current file only
          else if (atom.config.get('linter-terraform-syntax.format.currentFile'))
            fmtArgs.push(editorFile);

          // auto-format the target(s)
          this.helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), fmtArgs, { cwd: dir }).catch(error => {
        ...
        // execute the linting
        return this.helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), ['validate', '-no-color', '-json'], { cwd: dir, ignoreExitCode: true, timeout: atom.config.get('linter-terraform-syntax.timeout') * 1000 }).then(output => {
          const toReturn = [];
