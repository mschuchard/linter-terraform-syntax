        ...
        return helpers.exec(atom.config.get('linter-terraform-syntax.terraformExecutablePath'), args, {stream: 'stderr', allowEmptyStderr: true}).then(output => {
          ...
          output.split(/\r?\n/).forEach(function (line) {
            ...
            if (matches_syntax != null && matches_file != null) {
              toReturn.push({
                severity: 'error',
                excerpt: matches_syntax[3],
                location: {
                  file: file,
                  position: [[Number.parseInt(matches_syntax[1]) - 1, Number.parseInt(matches_syntax[2]) - 1], [Number.parseInt(matches_syntax[1]) - 1, Number.parseInt(matches_syntax[2])]],
                },
              });
            }
